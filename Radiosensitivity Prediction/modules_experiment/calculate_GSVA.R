...
mice_data <- TRUE # only set to true if you're using the original-mice data mix
data_name <- "rna_imputed" #"methylation_imputed"
number_of_modules <- "500" # options: "all", "most-enriched", int
module_grouping_method <- "gsva" #options: plage, gsva
...
if (mice_data) {
  data_dir <- "C:/Users/mmarc/Documents/code/P-Net-Reproducibility-Paper-Fork/Radiosensitivity Prediction/model_evaluation_on_mice_data/data/prepare_mice_data_for_gsva" # nolint: line_length_linter.
}
...
if (mice_data) {
  filename <- sprintf("mice_%s_%s.csv", data_name, number_of_modules)
}
...
# transpose if needed. We want genes as rows and samples as columns.
if (ncol(expr) > nrow(expr)) {
  expr <- t(expr)
}

# preprocessing for imputed methylation data. Crop values to [0, 1]
...
# Create a parameter object for GSVA (this is now required in new versions)
if (module_grouping_method == "gsva") {
  gsva_par <- gsvaParam(expr_filtered, gene_sets, kcdf = "Gaussian")
} else if (module_grouping_method == "plage") {
  gsva_par <- plageParam(expr_filtered, gene_sets)
} else if (module_grouping_method == "ssgsea") {
  gsva_par <- ssgseaParam(expr_filtered, gene_sets)
}
...
# Save result
filename <- sprintf("%s_scores_%s_%s.csv", module_grouping_method, data_name, number_of_modules) # nolint
if (mice_data) {
